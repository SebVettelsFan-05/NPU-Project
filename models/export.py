...
# ---------------------------------------------------------------------------
def wrap_items(items, width=88, indent="    "):
    """Join items with ", " and wrap on TOKEN boundaries.

    Slicing the joined string by character count splits literals in half
    (4'd12 -> "4'd1" + newline + "2"), which produces a file that does not
    compile.  Break between items instead.
    """
    lines, cur = [], ""
    for it in items:
...
def to_verilog(net: M.Net) -> str:
    ...
    for i, layer in enumerate(net.layers):
        flat = [int(layer.weight[oc, ic, ky, kx]) + M.WEIGHT_ZP
                for oc, ic, ky, kx in weight_order(layer)]
        wrapped = wrap_items([f"4'd{v}" for v in flat])
    ...
    word_wrapped = wrap_items([f"12'h{w:03X}" for w in words])
...
def to_c_header(net: M.Net, meta: dict) -> str:
    ...
    value, width = config_bits(net)
    wrapped = wrap_items([f"0x{w:04X}" for w in words], width=84)
